...
void	perlin_noise_texture(t_additional *s, void *t)
{
	...
	while (++i < pr->octaves)
	{
		tmp = create_vector(s->point.x * freq.x, s->point.y *
												freq.y, s->point.z * freq.z);
		coef[0] += calculate_perlin_noise(tmp) * coef[1];
		coef[1] *= pr->persistence;
		freq = create_vector(pr->frequency_att.x * freq.x, pr->frequency_att.y *
										freq.y, pr->frequency_att.z * freq.z);
	}

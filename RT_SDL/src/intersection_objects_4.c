...
/*   Updated: 2017/10/11 17:39:15 by askochul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv.h"

static double  check_paraboloid_limits(double t, t_ray *r, t_limited_paraboloid *c)
{
	t_vect tmp[2];
	t_vect  point;
	...
	{	point = intersection_point(t, r);
		...
		if (vector_dot_product(&c->dir, &tmp[0]) > PRECISION && vector_dot_product(&c->dir, &tmp[1]) < PRECISION)
			return (t);
	}
	return (-1);
}

int		intersection_limited_paraboloid(t_ray *r, void *paraboloid, double *t, double *z)
{
...
int		intersection_limited_sphere(t_ray *r, void *sphere, double *t, double *z)
{
	t_vect		dist;
	t_equation	n;
	t_limited_sphere	*s;

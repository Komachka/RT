...
int				intersection_limited_cylinder(t_ray *r, void *cyl, double *t)
{
	...
	if (c->caps == ON)
	{
		...
		ttm[0].r = c->r;
		intersection_disc(r, &ttm[0], &cap[2]);
		ttm[1].pos = c->p2;
		ttm[1].norm = c->p2_norm;
		ttm[1].r = c->r;
		intersection_disc(r, &ttm[1], &cap[3]);
	}
...
int				intersection_limited_cone(t_ray *r, void *cone, double *t)
{
	t_limited_cone	*cn;
	t_vect			tmp[2];
	t_vect			delta;

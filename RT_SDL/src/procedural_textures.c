...
void 	perlin_noise_texture(t_additional *s, void *t)
{
	...
	s->mat.cl = calculate_color(pow(coef, pr->exponent), &s->mat.cl);
}
...
void 	plasma_texture(t_additional *s, void *t)
{
	...
	s->mat.cl.r = 0.5 * (1.0 + cos(2.0 * M_PI * pr->frequency.r * n_sum + pr->phase.r));
	s->mat.cl.g = 0.5 * (1.0 + cos(2.0 * M_PI * pr->frequency.g * n_sum + pr->phase.g));
	s->mat.cl.b = 0.5 * (1.0 + cos(2.0 * M_PI * pr->frequency.b * n_sum + pr->phase.b));
}
...
void 	contours_texture(t_additional *s, void *t)
{
	...
	coef = 0.5 * (1 + sin(2.0 * M_PI * pr->frequency * calculate_perlin_noise(s->point)));
	if (coef > (1.0 - pr->t))
		s->mat.cl = pr->contour;
	else
...
void jupiter_texture(t_additional *s, void *t)
{
	...
	tt = 1.0 / calculate_perlin_noise(create_vector(s->point.x * 20.0, s->point.y * 20.0, s->point.z * 10.0));
	w = s->point.z + pr->vortices * tt;
	ss = calculate_perlin_noise(create_vector(s->point.x / 1000.0, s->point.y / 1000.0, pr->bands * w));
	s_ = ss + pr->perturbations * ((calculate_perlin_noise(create_vector(s->point.x * 50.0, s->point.y * 50.0, 25.0 * w))) - 0.5);

...
/*   Updated: 2017/09/28 20:54:57 by askochul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		intersection_ellipsoid(t_ray *r, void *ellipsoid, double *t)
{
	...
	n.a = ((r->dir.x * r->dir.x) / (e->size_x * e->size_x)) + ((r->dir.y *
		r->dir.y) / (e->size_y * e->size_y)) +
		((r->dir.z * r->dir.z) / (e->size_z * e->size_z));
	n.b = (2.0 * (r->origin.x - e->pos.x) * r->dir.x) / (e->size_x * e->size_x)
		+ (2.0 * (r->origin.y - e->pos.y) * r->dir.y) / (e->size_y * e->size_y)
		+ (2.0 * (r->origin.z - e->pos.z) * r->dir.z) / (e->size_z * e->size_z);
